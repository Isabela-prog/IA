...
Usar heuristica da distância em linha reta, 
busca gulosa (usa vetor ordenado):
    - olhar para adjascentes de Arad
    - Ver qual deles tem a menor distância entre eles e Bucharest
    - Olhar seus adjascentes, repitir o processo
obs.: quando Bucharest é um vértice de uma cidade, considerar a distância como zero
'''
...
class OrderedVector:
    ...
    def insert(self, vertex):
        ...
        for i in range(self.last_position + 1):
            position = i
            # Posição encontrada
            if self.values[i].objective_distance > vertex.objective_distance:
                break
            # Posicionar no fim
    ...
    def impress(self):
        if self.last_position == -1:
            ...
            for i in range(self.last_position + 1):
                print(i, ' - ', self.values[i].label, ' - ', self.values[i].objective_distance)
...
class GreedySearch:
    ...
    def search(self, actual):
        ...
        if actual == self.objective:
            ...
            # Pegar adjacentes de cada cidade
...
grafo = Grafo()
print(grafo.arad.print_adjacent(), '\n')
...
greedy_search = GreedySearch(grafo.bucharest)
print(greedy_search.search(grafo.arad))